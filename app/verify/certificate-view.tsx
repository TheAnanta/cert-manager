...
export default function CertificateView({ certificate, participant, template, event, placeholders }: any) {
    ...
    const certName = template?.name?.includes("SME") ? "Subject Matter Expert" : event?.name?.includes("CHITTI") ? "Operations Engineer" : event?.name || template?.name || 'Certificate of Completion';
    const orgName = event?.name?.includes("CHITTI") ? "CHITTI" : process.env.NEXT_PUBLIC_ORG_NAME || 'the ananta studio';
    
    const rawIssueDate = template?.name?.includes("SME") ? "2024-03-01T08:19:17.864Z" : event?.name?.includes("CHITTI") ? "2025-10-01T08:19:17.864Z" : certificate?.issuedAt || event?.startDate || new Date();
    ...
    const linkedInParamsObj: Record<string, string> = {
    ...
    if (isSmeOrChitti) {
        linkedInParamsObj.expirationYear = '2026';
        linkedInParamsObj.expirationMonth = '4';
    }

    const linkedInParams = new URLSearchParams(linkedInParamsObj);
